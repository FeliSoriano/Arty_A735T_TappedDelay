...
{
  /**
   * @brief This function initialices the UART communication. It asume only one port/instance
   * is available, and get's it's adress from the xparameters.h file.
   *
   * @param[in,out] uartLiteInstance. Variable where the UART Lite Instance will be saved.
   *
                         ...
                         XUartLite uartLiteInstance)
{
  ...
  /**
   * @brief Runs until a long enough msg (or a terminated one) is received via UART.
   *
   * @param[in, out] buffer Buffer to save the received msg in.
   * @param[in] max_len Maximum size of the received msg.
   * @param[in] uartLiteInstance. Uart Lite instance.
   *
   * @return Returns 1 if a msg with characters other than termination are received.
   */
...
/**
 * @brief This function is meant to be used as a hand-shake with the other end of the communication.
 * The program will pause until the 'continue' msg is received.
 *
 * @param[in] uartLiteInstance. Uart Lite instance.
 *
 * @return 1 if the 'continue' msg is received, 0 if a termination character is received instead.
 */
{
  char c = '0';
  while (c != CONTINUE)
...
uint8_t read_states(uint32_t baseaddr)
{
   ...
   *
   * @param[in] baseaddr. Base address where the flags are located.
   *
...
int readCHN(int chnNumber, uint64_t *dataBuffer, XUartLite uartLiteInstance)
{
   ...
   * @param[in,out] dataBuffer. Buffer where the data will be written
   * //TODO: move dataBuffer inside the function! (and delete pre)
   * @param[in] uartLiteInstance. Uart Lite instance.
   *
   * @pre dataBuffer size should be bigger than addr loop condition
   * @pre pray that someone set the channel to RUN before entering the function
   *
   * @post if BRAMSize addresses were read, starts clearing channel. This should always happen
   * since the function waits until the channel is full before reading.
   *
  ...
  uint32_t bram_base = CHN0_BRAMADDR; // eventually this will be smth like: CHANNELS_BRAM[chnNumber]
...
void rearmCHN(int chnNumber)
{
   ...
   *
   * @post Set's the TDC back to run. TODO: Think if it should be sent to IDLE instead
   */
  // uint32_t bram_base = CHN0_BRAMADDR;
  uint32_t ctrl_base = CHN0_BASEADDR; // eventually this will be smth like: CHANNELS_BRAM[chnNumber]
